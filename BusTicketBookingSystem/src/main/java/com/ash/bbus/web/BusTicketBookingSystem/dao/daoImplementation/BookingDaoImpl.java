...
import com.ash.bbus.web.BusTicketBookingSystem.dao.BookingDao;
...
public class BookingDaoImpl implements BookingDao {
	...
	public List<Booking> findByUserIdAndStatus(Long userId, BookingStatus status) {
		TypedQuery<Booking> query = entityManager.createQuery("SELECT b FROM Booking b " + "WHERE b.user.id = :userId "
				+ "AND b.status = :status " + "ORDER BY b.bookingTime DESC", Booking.class);
	...
	@Override
	public List<Booking> findByTripDate(LocalDate date) {
		TypedQuery<Booking> query = entityManager.createQuery(
				"SELECT b FROM Booking b " + "WHERE b.trip.travelDate = :date " + "ORDER BY b.bookingTime DESC",
				Booking.class);
	...
	public long countBookingsByTripId(Long tripId) {
		TypedQuery<Long> query = entityManager.createQuery(
				"SELECT COUNT(b) FROM Booking b " + "WHERE b.trip.id = :tripId " + "AND b.status = 'CONFIRMED'",
				Long.class);
