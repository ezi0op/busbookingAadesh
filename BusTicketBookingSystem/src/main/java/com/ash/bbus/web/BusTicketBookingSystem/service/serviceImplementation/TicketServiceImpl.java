...
import com.ash.bbus.web.BusTicketBookingSystem.repository.TicketRepository;
...
public class TicketServiceImpl implements TicketService {
	...
	public TicketServiceImpl(TicketRepository ticketRepository, BookingRepository bookingRepository,
			PassengerRepository passengerRepository, TripSeatRepository tripSeatRepository, PdfUtil pdfUtil) {
	...
	public TicketDTO generateTicket(Long bookingId) {
		...
		Ticket ticket = Ticket.builder().pnrNumber(pnr).booking(booking).valid(true).build();
	...
	public TicketDTO getTicketByPnr(String pnrNumber) {
		Ticket ticket = ticketRepository.findByPnrNumber(pnrNumber)
				.orElseThrow(() -> new ResourceNotFoundException("Ticket not found for PNR: " + pnrNumber));
		return buildTicketDTO(ticket, ticket.getBooking());
	}
	...
	private TicketDTO buildTicketDTO(Ticket ticket, Booking booking) {
		...
		List<TripSeat> seats = tripSeatRepository.findByBookingId(booking.getId());
		...
		dto.setSeats(seats.stream().map(s -> {
			...
			sDto.setSeatType(s.getSeat().getBus().getSeatType());
